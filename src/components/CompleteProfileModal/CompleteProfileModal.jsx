...
function CompleteProfileModal({
  ...
  const handleProfileSubmit = async (e) => {
    ...
    if (!token) {
      setErrorMessage("توکن احراز هویت معتبر نیست. لطفاً دوباره وارد شوید.");
      setIsLoading(false);
      return;
    }
    ...
    try {
      const response = await fetch(
        "http://127.0.0.1:8000/api/complete-profile",
        {
          ...
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`, // افزودن توکن برای احراز هویت
          },
          body: JSON.stringify({
            name: fullName,
            username: username,
      ...
      if (response.ok) {
        ...
        // بررسی پاسخ سرور برای پیام خطا
        const data = await response.json();
        console.log("Server Error Response:", data); // لاگ خطای سرور
        setErrorMessage(data.message || "خطایی رخ داده است.");
      }
    } catch (error) {
      console.error("Network Error:", error); // لاگ خطای شبکه
      setErrorMessage("ارتباط با سرور برقرار نشد. دوباره تلاش کنید.");
    } finally {
      setIsLoading(false);
    }
  ...
  if (!showCompleteProfileModal) return null; // اصلاح این خط برای نمایش درست مودال
          ...
          <div className="animate-fadeIn">
            <div className="flex flex-col justify-between items-center mb-4">
              ...
              <div>
                ...
                <p className="text-center text-gray-500 mb-10">
                  اطلاعات زیر را وارد کنید تا به امکانات کامل دسترسی داشته باشید.
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleProfileSubmit}>
              ...
              <Button
                type="submit"
                disabled={isLoading}
                className="w-full py-3 bg-primary-400 text-white rounded-md hover:bg-primary-500 hover:shadow-lg transition duration-300"
              >
                {isLoading ? "در حال ارسال..." : "ذخیره پروفایل"}
              </Button>
            </form>
          </div>
        )}
